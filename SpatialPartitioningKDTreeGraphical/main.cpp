...
#pragma endregion
	...
	for (int i = 0; i < entityCount; i++)
	{
		...
		randX = ((rand() % 201) - 100) / 100.0f;
		randY = ((rand() % 201) - 100) / 100.0f;
		...
		positions.push_back(entityList[i]->transform->position);
		tree->Insert(entityList[i]->transform->position, linePoints);
	}	
